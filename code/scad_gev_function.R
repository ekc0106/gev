...
#z_function
func_z <- function(dmatrix, mu, u, lam, rho, a){
  
  for(i in 1:length(z_init)){
    if(abs(z_init[i]) < lam){
      if(abs(dmatrix %*% mu + u)[i] <= (rho + 1)*lam/rho & abs(dmatrix %*% mu + u)[i] >= lam/rho){
        z_init[i] <- (dmatrix %*% mu + u)[i] - sign(dmatrix %*% mu + u)[i] * lam/rho
      }else if(abs(dmatrix %*% mu + u)[i] < lam/rho){
        z_init[i] <- 0
      }else{
        z_init[i] <- sign(dmatrix %*% mu + u)[i] * lam
      }
    }else if( abs(z_init[i]) > lam & abs(z_init[i]) <= a*lam ){
      if((rho*a - rho)/(rho*a - rho - 1) * abs(dmatrix %*% mu + u)[i] >= a/(rho*a - rho - 1) - a & 
         (rho*a - rho)/(rho*a - rho - 1) * abs(dmatrix %*% mu + u)[i] < a/(rho*a - rho - 1) - 1){
        z_init[i] <- (rho*a - rho)/(rho*a - rho - 1) * (dmatrix %*% mu + u)[i] -sign(dmatrix %*% mu)[i]* a*lam/(rho*a - rho - 1)
      }else if((rho*a - rho)/(rho*a - rho - 1) * abs(dmatrix %*% mu + u)[i] > a/(rho*a - rho - 1) - 1){
        z_init[i] <- -lam*sign(dmatrix %*% mu + u)[i]
      }else{
        z_init[i] <- -a*lam*sign(dmatrix %*% mu + u)[i]
      }
    }else{
      if(abs(dmatrix %*% mu + u) > a * lam){
        z_init[i] <- (dmatrix %*% mu + u)[i]
      }else{
        c(a*lam, -a*lam)[which.min(c((dmatrix %*% mu - a*lam + u)[i],(dmatrix %*% mu + a*lam + u)[i]))]
      }
    }
  }
  return(z_init)
